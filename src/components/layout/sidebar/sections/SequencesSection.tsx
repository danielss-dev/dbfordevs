...
import { CircleNotch, Trash, ArrowsClockwise, Copy, Hash } from "@phosphor-icons/react";
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <div>
            <TreeItem
              label="Sequences"
              icon={<Hash weight="regular" className="h-3.5 w-3.5 text-muted-foreground" />}
              level={1}
            ...
            >
              {isLoadingSequences ? (
                ...
                <div className="py-1.5 text-xs text-muted-foreground" style={{ paddingLeft: `${2 * 14 + 6}px` }}>No sequences found</div>
              ) : (
                <div className="flex items-center gap-2 py-1.5 text-xs text-muted-foreground" style={{ paddingLeft: `${2 * 14 + 6}px` }}>
                  <CircleNotch weight="regular" className="h-3 w-3 animate-spin" />
                  <span>Loading sequences...</span>
        ...
        <ContextMenuContent className="w-48">
          <ContextMenuItem onSelect={loadConnectionSequences} className="gap-2">
            <ArrowsClockwise weight="regular" className={cn("h-4 w-4", isLoadingSequences && "animate-spin")} />
            Refresh
          </ContextMenuItem>
        </ContextMenuContent>
      </ContextMenu>
