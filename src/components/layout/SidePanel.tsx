...
function FieldsPanel() {
  ...
  const isAutoIncrementColumn = (dataType: string, isPrimaryKey: boolean) => {
    const type = dataType.toLowerCase();
    return (
      type.includes("serial") || // PostgreSQL: serial, bigserial, smallserial
      type.includes("identity") || // MSSQL/Oracle: identity columns
      (type.includes("int") && isPrimaryKey) // Common pattern: int PK often auto-increment
    );
  };

  const fields = mergedColumns.map(col => {
    const isAutoIncrement = isAutoIncrementColumn(col.dataType, col.isPrimaryKey);
    return {
      name: col.name,
      type: col.dataType,
