...
pub async fn execute_query(request: QueryRequest) -> Result<QueryResult, AppError> {
    ...
    let any_pool = manager.get_any_pool(&request.connection_id).await?;
    ...
    driver.execute_query(&any_pool, &sql).await
}
...
pub async fn get_tables(connectionId: String) -> AppResult<Vec<TableInfo>> {
    ...
    let driver = get_driver(&config);
    let any_pool = manager.get_any_pool(&connectionId).await?;
    
    driver.get_tables(&any_pool).await
}
...
pub async fn get_table_schema(
    ...
    driver.get_table_schema(&any_pool, &tableName).await
}
...
pub async fn insert_row(
    ...
    driver.execute_query(&any_pool, &sql_with_values).await
}
